...
#I also tried this another way
    # for rain in results:
    #     date_rain={}
    #     if rain.date is not None:
    #         date_rain["date"]=rain.date
    #     if rain.prcp is not None:
    #         date_rain["Rain Amount"]=rain.prcp
    #     if type(rain.prcp) == float:
    #         precipitation_amt.append(rain.prcp)
    #     both_values.append(date_rain)
    # total_rain=round(sum(precipitation_amt),3)
    # total_rain1={"Total Rain":total_rain}
    
    # return jsonify(both_values,total_rain1)
